...
var appPath = process.cwd();   // path to the project (may be a file)
var appDir = process.cwd();    // path to the project folder (may be the same as appPath)
var pidPath = joinPath(appDir, '.andever.pid');
var stdio = 'ignore';
var appName;

function getAppName() {
	try {
		const pkg = JSON.parse(fs.readFileSync(joinPath(appDir, 'package.json'), 'utf8'));
		if (pkg.name) {
			return pkg.name;
		}
	} catch (error) { /* ignore */ }
...
exports.setOptions = function (opts) {
	if (opts.appPath) {
		...
		var stats = fs.statSync(appPath);
		appDir = stats.isDirectory() ? appPath : dirname(appPath);

		pidPath = joinPath(appDir, '.andever.pid');
		appName = getAppName();
	}

	if (opts.pidPath) {
		pidPath = resolvePath(appDir, opts.pidPath);
	}
